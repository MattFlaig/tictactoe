...
function computerNextRound(fieldNumber) {
  if(gameRound<=1){
    deletePossibleChoices(fieldNumber);
    var computerChoice = randomChoice(fields);
  }
  else {
...
function egocentricStrategy(fieldNumber){
    ...
    alert("fields: " + fields);
    alert("possible Choices: " + possibleChoices);
      if (gameRound > 3){
...
function findAlternativeChoices(){
  ...
  else{nextChoice = getNextChoice(fields);}
  return nextChoice;
}
...
function searchForWin(counter){
  ...
  if(possibleChoices.length>0){
    for(i=counter;i<possibleChoices.length;++i){
